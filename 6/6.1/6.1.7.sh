...
# Check the current permissions, owner, and group of /etc/gshadow
current_status=$(stat -Lc "%a %u %U %g %G" /etc/gshadow)
echo "Current status of /etc/gshadow: $current_status"
...
if [[ "$current_perms" == "$expected_perms" && "$current_uid" == "$expected_uid" && "$current_gid" == "$expected_gid" ]]; then
    echo "The /etc/gshadow file is already configured correctly."
else
    echo "The /etc/gshadow file is not configured correctly."
